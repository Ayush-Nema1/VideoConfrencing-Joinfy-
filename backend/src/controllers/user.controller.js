...
const login = async(req,res)=>{
    ...
    if(!username || !password) {
        return res.status(400).json({message : "please provide essential details"})
    }
    try{
...
const register = async(req,res)=>{
    ...
    res.status(httpStatus.CREATED).json({message: "User registered successfully"});
           ...
           res.json({message : `Somethng went wrong ${e}`});
    }
}
...
export {login,register,addToUserHistory,getUserHistory};